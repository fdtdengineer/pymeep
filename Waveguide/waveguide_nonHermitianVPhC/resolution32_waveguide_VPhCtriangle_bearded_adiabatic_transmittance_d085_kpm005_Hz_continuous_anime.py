...
def phc_trans(PhC = True, lengthPhC = 100, decay_check=0, T_decay=500, name_time="hogehoge"):
    ...
    sources = [mp.Source(mp.ContinuousSource(frequency=fcen),
                     component=mp.Hz,
                     center=mp.Vector3(-length/2 +1,0), size=mp.Vector3(0,wgi*np.sqrt(3)))]
    ...
    #sim.run(until_after_sources=mp.stop_when_fields_decayed(50, mp.Hz, mp.Vector3(decay_check), 1e-15))
    ...
    sim.run(mp.at_every(T,Animate),until=T_decay)
    plt.close()
    filename = name_time + "_" + geometry + ".mp4"
...
if __name__ == "__main__":  
    ...
    a = 470
    c_const = 299792458
    #phc_trans(PhC = False, lengthPhC = 20, decay_check=10, T_decay=500, name_time=name_dt)
    phc_trans(PhC = True, lengthPhC = 50, decay_check=20, T_decay=1000, name_time=name_dt)
