...
def phc_trans(PhC = True, lengthPhC = 100, decay_check=0, T_decay=500, name_time="hogehoge"):
    ...
    if PhC:
        ...
        blk_above = mp.Block(mp.Vector3(lengthPhC,widthPhC*np.sqrt(3),mp.inf),
                                center=mp.Vector3(0, widthPhC*np.sqrt(3)/2 - np.sqrt(3)/4*0 + 0*(yshift + 1/(2*np.sqrt(3))), 0),
    ...
    plt.savefig(name_time + "_Hz_" + geometry + "_imwshow.png")
    #plt.axis('off')
    plt.show()
...
if __name__ == "__main__":  
    ...
    phc_trans(PhC = True, lengthPhC = 50, decay_check=20, T_decay=10000, name_time=name_dt)
